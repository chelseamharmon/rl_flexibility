function [S,Q] = genlouvain(B,limit)
...
%
%   [S,Q] = GENLOUVAIN(B,limit) with B a function handle such that B(i) returns
...
%         disp([num2str(length(unique(y))),' ',num2str(Q)])
        ...
        for i = 1:length(M(1))    %loop over nodes in pseudorandom order     

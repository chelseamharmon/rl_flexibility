...
if [ -e $1 ];
	...
	if [ -e $2 ];
		...
		sed -e 's:XXOUTPUTXX:'$out':g' -e 's:XXTRXX:'$TR':g' -e 's:XXVOLSXX:'$vols':g' -e 's:XX4DDATAXX:'$filtdata':g' -e 's:XXCONFXX:'$conf':g' -e 's:XXSTANDARDPATHXX:'standard':g' $3 > $(dirname $filtdata)/$(basename $fsf);
		feat $(dirname $filtdata)/$(basename $3) #run file
		#rm -rf tmp.fsf
